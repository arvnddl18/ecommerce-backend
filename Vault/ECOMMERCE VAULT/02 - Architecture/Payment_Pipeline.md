...
Tags: #architecture #stripe #payments #marketplace
...
sequenceDiagram
    ...
    API->>Stripe: Create Checkout Session (PaymentIntent with transfer_group)
    ...
    Worker->>Stripe: Create Transfers to Seller Connected Accounts (minus platform fee)
```

## 2. Key Safeguards & Marketplace Considerations
- **Zero Cardholder Data:** Server never touches raw card data; handled entirely by Stripe Elements / Hosted Checkout.
...
- **Platform Application Fee:** Platform retains its agreed commission percentage on each transaction before vendor payout disbursement.
